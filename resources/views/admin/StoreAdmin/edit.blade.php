            ...
            <div class="tab-content">
              <div class=" active tab-pane" id="profile">
                 {{--  @if (count($errors) > 0)
                  ...
                  @endif --}}
                <!-- The timeline -->
                    <form action="{{ url('admin/StoreAdmin/updateShowRoomAdmin/'.$ShowRoomAdmin->myid) }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
                   {{-- {!! Form::open(array('url' =>'admin/StoreAdmin', 'method'=>'post', 'class' => 'form-horizontal', 'enctype'=>'multipart/form-data')) !!} --}}
                      ...
                      {{-- <div class="form-group">
                        ...
                        <div class="col-sm-10">
                           <select class="form-control zoneContent" name="state">
                                ...
                                @if($ShowRoomAdmin->state != "")
                                    @foreach($result['zones'] as $zones)
                                        <option @if($ShowRoomAdmin->state==$zones->zone_id) selected @endif value="{{ $zones->zone_id }}">{{ $zones->zone_name }}</option>
                                    @endforeach
                                @endif
                            </select>
                      ...
                      <div class="form-group">
                        <label for="category" class="col-sm-2 control-label">{{ trans('labels.proCategory') }} <span class="clsred">*</span></label>
                        <div class="col-sm-10">
                        <select class="field select2 form-control" name="category[]" autofocus required id="category" multiple="multiple" data-placeholder="{{ trans('labels.selectCategoryType') }}">
                          <?php $ShowRoomAdmin->category = explode(',', $ShowRoomAdmin->category); ?>
                            @forelse($result['procategory'] as $pro_cat) 
                                <option value="{{ $pro_cat->id }}" @if(in_array($pro_cat->id, $ShowRoomAdmin->category)) {{ 'selected="selected"' }} @endif >{{ $pro_cat->name }}</option>                                  
                            @empty    
                            @endforelse  
                        </select>
                        </div>
