...
$(document).ready(function () { 
    $('#tabuserid').DataTable({
        ...
        "serverSide": true,
        "ajax":{
                 "url": "{{ url('/admin/StoreAdmin/allsearch') }}",
                 "dataType": "json",
                 "type": "POST",
    ...
    <div class="box-header-main">
        <h3 class="page-title">{{ trans('labels.StoreAdmin') }} </h3>    
        <p><a href="{{ url('/admin/StoreAdmin/create') }}" class="btn btn-success"><i class="fa fa-edit" aria-hidden="true"> </i>{{ trans('labels.addNew') }}</a>
         <a class="btn btn-danger deletesellected" > <i class="fa fa-trash" aria-hidden="true"></i>{{ trans('labels.delete') }}</a> </p>
    </div>
    
    <div class="panel panel-default">
        <div class="panel-heading">
           {{ trans('labels.StoreAdmin') }}
        </div>
    ...
    $('.deletesellected').click(function(){ 
        ...
        if(checkValues !='')
        {
            ...
            {   return false;  }
                $.ajax({
                    type:'POST',
                    url:'{{ url("admin/StoreAdmin/alldelete") }}',
