...
function filterGlobal () {
    ...
    $(document).ready(function () { 
        $('#tabuseridpos').DataTable({
            ...
            "serverSide": true,
            "ajax":{
                     "url": "{{ route('admin.fueltype.allposts') }}",
                     "dataType": "json",
                     "type": "POST",
    ...
    <div class="box-header-main"><h3 class="page-title">{{ trans('labels.fuelType') }}</h3>    
    <p><a href="{{ route('admin.fueltype.create') }}" class="btn btn-success"><i class="fa fa-edit" aria-hidden="true"> </i>{{ trans('labels.addNew') }}</a>
     <a class="btn btn-danger deletesellected" > <i class="fa fa-trash" aria-hidden="true"></i> {{ trans('labels.delete') }}</a> </p></div>
    
    
    <div class="panel panel-default">
        <div class="panel-heading">
           {{ trans('labels.fuelTypeList') }}
        </div>
    ...
    $('.deletesellected').click(function(){ 
        ...
        if(checkValues !='')
        {
            ...
            {   return false;  }
                $.ajax({
                    type:'POST',
                    url:'{{ route("admin.fueltype.alldelete") }}',
