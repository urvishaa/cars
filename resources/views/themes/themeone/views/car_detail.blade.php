...
<?php 
  ...
  if(@$car_Detail->car_name != '')
  {
     $nname=@$car_Detail->car_name;
  }
  else
  {
    $nname= "Iraq car";
  ...
  if(count(@$car_img) > 0) 
  {
    // echo public_path().'/carImage/'.@$car_img[0]->img_name;
    if(file_exists(public_path().'/carImage/'.@$car_img[0]->img_name)  && @$car_img[0]->img_name != '')
    {
      $imgg= asset('').'public/carImage/thumb_'.$car_img[0]->img_name;
    }
    else
    {
      $imgg= URL::to('/public/default-image.jpeg'); 
    }
  }
  else
  {
  ...
  if(@$car_Detail->description != '')
  {
    $desc=@$car_Detail->description;
  }
  else
  {
    $desc= "Iraq car";
...
@section('title')<?php echo $nname; ?>@endsection
@section('iamge')<?php echo $imgg; ?>@endsection
@section('description')<?php echo $desc; ?>@endsection 
            ...
            <div class="row">
                ...
                <div class="col-lg-4">
                    <div class="sidebar-content-wrap m-t-50">
                      ...
                      <?php } ?>
                      @if($car_Detail->pro_type == '1')
                           <div class="car-det-inr">
                             <div class="car-det-inr left">
                                <h4>{{ trans('labels.salePrice') }}</h4> 
                             </div>
                             <div class="car-det-inr right">
                                 <h4><?php 
                                 if ($car_Detail->sale_price != "") {
                       ...
                       @elseif($car_Detail->pro_type == '2') 

                       		@if($car_Detail->daily_rentprice != '' AND $car_Detail->daily_rentprice > 0) 
	                        <div class="car-det-inr">
	                            <div class="car-det-inr left">
	                                <h4>{{ trans('labels.dailyRentPrice') }}</h4> 
	                            </div>
	                            <div class="car-det-inr right">
	                                 <h4>{{ $car_Detail->daily_rentprice }}</h4>
	                            </div>
	                        </div> 
	                        @endif   

	                        @if($car_Detail->weekly_rentprice != '' AND $car_Detail->weekly_rentprice > 0)
	                        <div class="car-det-inr">
	                            <div class="car-det-inr left">
	                                <h4>{{ trans('labels.weeklyRentPrice') }}</h4> 
	                            </div>
	                            <div class="car-det-inr right">
	                                 <h4>{{ $car_Detail->weekly_rentprice }}</h4>
	                            </div>
	                        </div>  
	                        @endif  

	                        @if($car_Detail->month_rentprice != '' AND $car_Detail->month_rentprice > 0)
	                        <div class="car-det-inr">
	                            <div class="car-det-inr left">
	                                <h4>{{ trans('labels.monthRentPrice') }}</h4> 
	                            </div>
	                            <div class="car-det-inr right">
	                                 <h4>{{ $car_Detail->month_rentprice }}</h4>
	                            </div>
	                        </div>    
	                        @endif
