...
@section('content')
  ...
  <section class="content-header">
    <div class="box-header-main"><h3 class="page-title"> {{ trans('labels.AdminProfile') }} </h3></div>
  </section>
  <?php 
$urlnew = url(''); 
                ...
                <!-- The timeline -->
                   {!! Form::open(array('url' =>'admin/updateProfile', 'method'=>'post', 'class' => 'form-horizontal', 'enctype'=>'multipart/form-data')) !!}
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-sm-10">
                          {!! Form::text('email', auth()->guard('admin')->user()->email, array('class'=>'form-control', 'id'=>'email'))!!}
                          <span class="help-block" style="font-weight: normal;font-size: 11px;margin-bottom: 0;">
                      ...
                      <div class="form-group">
                        ...
                        <select class="field select2 form-control" name="city[]" autofocus required id="mainProId" multiple="multiple" data-placeholder="{{ trans('labels.selectCity') }}">
                                 @forelse($result['city'] as $pro_cat) 
                                    @if(isset($setCar) !='')
                                      <option value="{{ $pro_cat->id }}"  @if(in_array($pro_cat->id, $setCar)) {{ 'selected' }} @endif >{{ $pro_cat->name }}</option>
                                    @else
