...
/*function filterGlobal () {
    ...
    $(document).ready(function () { 
        $('#tabuseridpos').DataTable({
            ...
            "serverSide": true,
            "ajax":{
                     "url": "{{ url('admin/testimonials/allposts') }}",
                     "dataType": "json",
                     "type": "POST",
    ...
    <div class="box-header-main"><h3 class="page-title">{{ trans('labels.testimonials') }}</h3>    
    <p><a href="{{ url('/admin/testimonials/create') }}" class="btn btn-success"><i class="fa fa-edit" aria-hidden="true"> </i>{{ trans('labels.addNew') }}</a>
     <a class="btn btn-danger deletesellected" > <i class="fa fa-trash" aria-hidden="true"></i> {{ trans('labels.delete') }}</a> </p></div>
    
    
    <div class="panel panel-default">
        <div class="panel-heading">
           {{ trans('labels.Testimonials List') }}
        </div>
...
$( document ).ready(function() {
    $('.deletesellected').click(function(){ 
        ...
        if(checkValues !='')
        {
            var res=confirm('Are you Sure you want to delete testimonials?');
            if(res=="flase")
            {   return false;  }
                $.ajax({
                    type:'POST',
                    url:'{{url("/admin/testimonials/testimonialsalldelete") }}',
        ...
        else   {  alert('Select atleast one testimonials'); }
    });
});
