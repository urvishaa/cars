...
<?php endif; ?>
	...
	jQuery(document).on('click', '.is_liked_perso', function(e){
		var products_id = jQuery(this).attr('products_id');
		var selector = jQuery(this);
		jQuery('#loader').css('display','flex');	
		var user_count = jQuery('#wishlist-count').html();		
		jQuery.ajax({			
			...
			success: function (res) {			
				//jQuery('.head-cart-content').html(res);	
				var obj = JSON.parse(res);	
				var message = "Please login first to personalize this product.";
...
jQuery(document).on('click', '.add-to-Cart', function(e){
	...
	var vals = jQuery(this).attr('products_id');

	jQuery('#loader').css('display','flex');
	
	var formData = jQuery(".add-Product-form"+vals).serialize();
...
function getQuantity(){
	...
	jQuery.ajax({
		...
		success: function (res) {
			...
			if(res.purchasePrice > 0)
			{
				jQuery('.total_price').html('$'+res.purchasePrice);
				jQuery('#products_price').val(res.purchasePrice);

				var qty = jQuery('.qty').val();
				var total_price = (parseFloat(qty) * parseFloat(res.purchasePrice)); 
			   jQuery('.total_price').html('<?=$web_setting[19]->value?>'+total_price.toFixed(2));
			}
