...
class DefaultController extends Controller
{
	...
	public function contactstore(Request $request)
    {
       ...
       ), function($message) use ($request)
       {
          ...
          $message->to('info@iraqcars.net', 'Admin')->subject($request->get('subject'));
       });
    ...
    { 
		...
		{ 
			...
			if(count($top_car) > 0)
			{
				...
				$top_result = Car::whereIn('id',$resultstr)->get();
			...
			$maxprice = $request->input('maxprice');
					...
					if(count($top_car) > 0)
					{
					$result->whereNotIn('id',$resultstr); 
					} 
