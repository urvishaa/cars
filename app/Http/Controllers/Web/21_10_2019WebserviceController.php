	     ...
	     /*     if(!empty($post['type']))
            ...
            if(!empty($results))
            {
              ...
              $result = array();       
                ...
                $response = array('success' => 1, 'message' => trans('labels.carSuccessfully') ,'totalCount' => $countdata ,'result' => $re);
                echo json_encode($response,JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_UNESCAPED_UNICODE|JSON_HEX_AMP);exit;
            ...
            $carbook =array();
              foreach($carBooking1 as $carBooking)
              {
                  ...
                  if(count($carImagedata)){
                    ...
                    {                     
                      $uploadImg = $carimg->img_name ? $carimg->img_name : ''; 
                      $carbook['carImg'][]=$new.'/public/carImage/'.$uploadImg;                     
                    } 
                  }  
                  else
                  {
