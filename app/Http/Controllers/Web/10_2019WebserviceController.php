...
class WebserviceController extends Controller
{
  ...
  public function allCarList(Request $request)  // All Car List for userType == 1 and only for user
  {
        ...
        try
        {
            ...
            $data->join('specification','properties.specification','=','specification.id','left');
            $data->select('properties.*','car_brand.name as carBrand','property_category.name as procategory','property_category.id as catid','property_types.id as typeid','property_types.name as typename','specification.name as specificationName','fueltype.name as fueltypeName');
            ...
            $data1->join('specification','properties.specification','=','specification.id','left');
            $data1->select('properties.*','car_brand.name as carBrand','property_category.name as procategory','property_category.id as catid','property_types.id as typeid','property_types.name as typename','specification.name as specificationName','fueltype.name as fueltypeName');
            ...
            if(!empty($results))
            {
              ...
              $result = array();       
                ...
                {                           
                    ...
                    $result['specificationName'] = $value->specificationName;
  ...
  public function userCarList(Request $request)  // user Car List for userType == 1 and only for user //login user
  {
      ...
      $post = json_decode($input, true);  
        ...
        try
        {
            ...
            $results = DB::table('properties')->join('fueltype','fueltype.id','=','properties.fueltype','left')->join('specification','properties.specification','=','specification.id','left')->join('property_category','property_category.id','=','properties.prop_category','left')->join('car_brand','car_brand.id','=','properties.car_brand','left')->join('property_types','property_types.id','=','properties.pro_type','left')->select('properties.*','car_brand.name as carBrand','property_category.name as procategory','property_category.id as catid','property_types.id as typeid','property_types.name as typename','specification.name as specificationName','fueltype.name as fueltypeName')->where('properties.userType',1)->where('properties.userId',$post['userId'])->orderby('properties.id','DESC')->offset($post['startLimit'])->limit(10)->paginate();
  ...
  public function adminCarList(Request $request)  // admin car List for userType == 2 and only for adminshowroom
  {
      ...
      $post = json_decode($input, true);  
        ...
        try
        {
            ...
            $results = DB::table('properties')->join('fueltype','fueltype.id','=','properties.fueltype','left')->join('specification','properties.specification','=','specification.id','left')->join('property_category','property_category.id','=','properties.prop_category','left')->join('car_brand','car_brand.id','=','properties.car_brand','left')->join('property_types','property_types.id','=','properties.pro_type','left')->select('properties.*','car_brand.name as carBrand','property_category.name as procategory','property_category.id as catid','property_types.id as typeid','property_types.name as typename','specification.name as specificationName','fueltype.name as fueltypeName')->where('properties.userType',2)->where('properties.showRoomId',$post['adminId'])->orderby('properties.id','DESC')->offset($post['startLimit'])->limit(10)->paginate();
              //echo "<pre>"; print_r($results); die;
            if(count($results)>0)
            {
              $re= array();
  ...
  public function carDetail(Request $request) //car Deatil
  {
        ...
        {   
            ...
            $results = DB::table('properties')->select('properties.*','car_brand.name as carBrand','car_brand.id as carbrandId','property_types.name as typename','property_category.name as modelName','property_category.id as modelId','fueltype.name as fueltypeName','fueltype.id as fueltypeId','city.name as cityName','city.id as cityId','specification.name as specificationName')
              ...
              ->join('specification','properties.specification','=','specification.id','left')
              ...
              $result = array();        
                ...
                //{ 
                    ...
                    $showRoomAdmin = DB::table('administrators')->select('*')->where('myid', $results->showRoomId)->first();
                      ...
                      $result['specificationName'] = $results->specificationName;     
  ...
  { 
        ...
        try
        {    
            if((empty($post)) || (empty($decode->brand)) ||  (empty($decode->type)) || (empty($decode->userId)) || (empty($decode->model)) || (empty($decode->kilometer)) || (empty($decode->specification)) || (empty($decode->description))  || (empty($decode->location)) || (empty($decode->price)) ||  (empty($decode->fueltype)) || (empty($decode->city)) || (empty($decode->email))  || (empty($decode->phone)) || (!isset($decode->carId)) || (!isset($decode->year)) )
            { 
                $response = array();
                $response['success'] = 0;
  ...
  public function getcompanyCity(Request $request)  // admin List for userType == 2 and only for adminshowroom
   {
      ...
      $post = json_decode($input, true);  
        ...
        try
        {
            ...
            $results = DB::table('properties')->join('property_category','property_category.id','=','properties.prop_category','left')->join('car_brand','car_brand.id','=','properties.car_brand','left')->join('property_types','property_types.id','=','properties.pro_type','left')->join('fueltype','fueltype.id','=','properties.fueltype','left')->join('specification','properties.specification','=','specification.id','left')->select('properties.*','car_brand.name as carBrand','property_category.name as procategory','property_category.id as catid','property_types.id as typeid','property_types.name as typename','fueltype.name as fueltypeName','specification.name as specificationName')->where('properties.userType',3)->where('properties.companyId',$post['companyId'])->orderby('properties.id','DESC')->offset($post['startLimit'])->limit(10)->paginate();
              //echo "<pre>"; print_r($results); die;
            if(count($results)>0)
            {
              $re= array();
