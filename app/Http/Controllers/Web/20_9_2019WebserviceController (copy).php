...
class WebserviceController extends Controller
{
  ...
  public function allCarList(Request $request)  // All Car List for userType == 1 and only for user
  {
        ...
        try
        {
            ...
            $data->select('properties.*','car_brand.name as carBrand','property_category.name as procategory','property_category.id as catid','property_types.id as typeid','property_types.name as typename');
  ...
  { 
        ...
        try
        {    
            if((empty($post)) || (empty($decode->brand)) ||  (empty($decode->type)) || (empty($decode->userId)) || (empty($decode->model)) || (empty($decode->kilometer)) || (empty($decode->specification)) || (empty($decode->description))  || (empty($decode->location)) || (empty($decode->price)) ||  (empty($decode->fueltype)) || (empty($decode->city)) || (empty($decode->email))  || (empty($decode->phone)) || (!isset($decode->carId)) )
            { 
                $response = array();
                $response['success'] = 0;
  ...
  public function getcompanyCity(Request $request)  // admin List for userType == 2 and only for adminshowroom
   {
      ...
      $post = json_decode($input, true);  
        ...
        try
        {
            ...
            $results = DB::table('properties')->join('property_category','property_category.id','=','properties.prop_category','left')->join('car_brand','car_brand.id','=','properties.car_brand','left')->join('property_types','property_types.id','=','properties.pro_type','left')->select('properties.*','car_brand.name as carBrand','property_category.name as procategory','property_category.id as catid','property_types.id as typeid','property_types.name as typename')->where('properties.userType',3)->where('properties.companyId',$post['companyId'])->orderby('properties.id','DESC')->offset($post['startLimit'])->limit(10)->paginate();
              //echo "<pre>"; print_r($results); die;
            if(count($results)>0)
            {
              $re= array();
